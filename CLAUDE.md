...
MNML の全資料（HTML/スライド/レポート/提案書/設計書）は全社資料デザイン基準に準拠すること。
正本: MNML-LLC/mnml-agents `docs/document-design-standard.md`（CEO確定 2026-08-18）。要点:

- 基本理念: Less is more。装飾で盛らず余白と構造で導く。AI生成デフォルト配色（クリーム＋テラコッタ等）に流れない
- 配色: 白背景 `#ffffff` に黒基調 `#222`/`#333`。アクセントは1〜3色（既定: 青 `#0057b7`・朱 `#d95c5c`）。白黒コピーでも意味が伝わること
- タイポグラフィ: h1 1.6〜1.8em/700、h2 1.2〜1.3em/600、h3 1.05〜1.1em/600、本文 0.95〜1.0em/400、注記 0.8〜0.85em。行間 1.6〜1.75（スライド見出しは 1.2〜1.3）。1資料内で和文フォント混在禁止
- 余白: 8 の倍数スケール（4/8/12/16/24/32/48/64/96px）のみ。章の区切りは罫線でなく余白 48〜64px
- 見出しへの絵文字（💡🎯📊等）の一律付与は禁止。斜体を多用しない
- ページ幅: 読み物 820〜1000px / ダッシュボード 1200px
- シグネチャ: 1資料に1つ、記憶に残る強い要素（全体像1枚・扉章図解等）を置く
