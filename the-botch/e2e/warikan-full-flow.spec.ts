...
test.describe('割り勘フルフロー: 作成 → 立替 → 精算 → クローズ', () => {
  ...
  test('全フロー: 新規作成 → 立替×2 → 精算確定 → 一括完了 → CLOSED', async ({ page }, testInfo) => {
    ...
    //   UI 上のバッジが「クローズ」になった直後でも、Vercel Postgres の
    //   read-after-write 一時不整合（別 serverless インスタンス/pg プール経由の
    //   読み取りで書き込みが未反映になる稀ケース）で PAYING が返ることがあるため
    //   ポーリングする。Issue #281 の flaky 原因分析で 10s では稀に不足するため 20s に拡張。
    // ---------------------------------------------------------------------
    await expect
      .poll(
        async () => {
